...
/**
 * @brief Construct a Page by reading directly from a file path.
 *        Does NOT require a tableCatalogue entry.  Used for temporary
 *        sort-run pages that live outside the catalogue.
 *
 * @param pageName   full path, e.g. "../data/temp/SORT_R_R0_Page0"
 * @param columnCount  number of int columns per row
 * @param rowCount     number of rows stored in the file
 */
Page::Page(string pageName, int columnCount, int rowCount)
{
    logger.log("Page::Page(temp)");
